...
2026-01-28T05:30:04.388Z is the date. Write a haiku about it.

# System Prompt

x-anthropic-billing-header: cc_version=2.1.21.0ff; cc_entrypoint=cli;
...
<env>
Working directory: /private/var/folders/y2/ph4wxddj7_10b_vt67r8v_v80000gn/T/claude-history-1769578203176-h6q0zi
...
`/private/tmp/claude/-private-var-folders-y2-ph4wxddj7-10b-vt67r8v-v80000gn-T-claude-history-1769578203176-h6q0zi/774642fd-dd9c-4efd-b1e7-09caefbef0b2/scratchpad`
...
Available skills:
- advise: Query project learnings for relevant past experiments, failure patterns, and working configurations before starting work. Use at the beginning of a task, when stuck on a problem, or when you want to check if something has been tried before.
- claude-memory: Extract user preferences, rules, and corrections from the session and save them to CLAUDE.md with full observability. Use when the user shares preferences, corrections, or rules they want you to remember.
- commit-push: Commit and push changes to the current branch
- commit-push-pr: Commit, push, and open a pull request. Shortcut for /commit-commands:commit-push-pr
...
- image: Generate any image - icons, assets, illustrations, app icons. Specify what you need, output path, and whether transparency is required.
- mobile-copy: Write clear UI copy for mobile apps. Use when writing buttons, errors, empty states, permissions, onboarding, paywalls, or reviewing existing app text.
- multi-claude: Orchestrate child Claude sessions to implement complex multi-step plans. Use when you want to delegate focused tasks to separate Claude instances to save context window and maintain clean separation of concerns.
- nanobanana-skill: Generate images from text OR edit existing images using image prompting (input image + text). Uses Google Gemini API. Supports multiple input images for style transfer.
- paywall-copy: Write high-converting paywall copy for mobile subscription apps. Use when writing paywall text, CTA buttons, trial messaging, pricing copy, or reviewing existing paywall copy. Based on research from Superwall, RevenueCat, Apphud, and Purchasely.
- plan-feature: Create high-level implementation plans for complex features requiring multiple agent sessions
- retrospective: Extract learnings from the current session and append them to the project learnings file. Use at the end of a work session, after completing a task, or when you've discovered something important (a failure, a success, a workaround, or a pattern).
- session-id: Get the current Claude Code session ID from history
